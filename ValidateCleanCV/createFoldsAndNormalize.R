...
meanImputation = function(listOfDatasets){
  ...
  for(i in 1:length(listOfDatasets$Training)){
    ...
    Ntraining = nrow(train)
    data = rbind(train,test)
    #fullRank =T drops one of the extra columns for the one-hot encoding.
    oneHotEncoder = dummyVars("~.",data = rbind.data.frame(test,train)[-timeDeltaInd], fullRank = T, drop2nd = T)
    dataEncoded = predict(oneHotEncoder, data[-timeDeltaInd])
    trainEncoded = dataEncoded[1:Ntraining,]
    testEncoded = dataEncoded[(Ntraining+1):nrow(dataEncoded),]
